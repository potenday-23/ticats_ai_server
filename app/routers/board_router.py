...
@router.post("", response_model=BoardResponseSchema, summary="게시판 Create",
             description="- name, public는 필수로 입력해야하는 값입니다.")
...
@router.delete("/{board_id}", status_code=204, summary="게시판 Delete",
               description="- 내가 작성한 게시판만 삭제할 수 있습니다.")
...
@router.get("/{board_id}", response_model=BoardResponseSchema, summary="게시판 Get(1개)",
            description="- 내가 작성한 게시판 또는 전체공개 게시판(public=true)만 조회할 수 있습니다.")
...
def get_board_list(db: Session = Depends(get_db), user_id: int = Depends(UserIdProvider()), page: int = 1, size: int = 10):
